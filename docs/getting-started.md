...
根目录已经自带 [compose.yml](/Users/chenzhixin/mywork/agent/metapi/compose.yml)，直接改 `.env` 即可。
...
ACCOUNT_CREDENTIAL_SECRET=replace-with-strong-random-secret
METAPI_PORT=4000
```
...
- 前端地址：`http://localhost:4120`（Vite dev server）
...
| 本地开发 | `http://localhost:4120` | `http://localhost:4000` |
