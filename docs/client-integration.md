...
| **API Key** | 你设置的 `PROXY_TOKEN` 值，或创建的[下游 API Key](./configuration.md#3-管理后台下游密钥) |
...
除了全局 `PROXY_TOKEN`，你还可以创建多个项目级下游 Key。每个 Key 可独立设置启停状态、站点分配策略，并按需绑定模型群组；留空模型群组表示不额外限制模型。这个模式适用于多团队/多项目共用一个 Metapi 实例的场景。

详细字段说明见 [配置说明 → 管理后台「下游密钥」](./configuration.md#3-管理后台下游密钥)。
