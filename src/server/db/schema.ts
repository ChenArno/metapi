...
export const sitePools = sqliteTable('site_pools', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  name: text('name').notNull(),
  ...
  nameIdx: index('site_pools_name_idx').on(table.name),
  enabledIdx: index('site_pools_enabled_idx').on(table.enabled),
}));
...
export const sitePoolMembers = sqliteTable('site_pool_members', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  poolId: integer('pool_id').notNull().references(() => sitePools.id, { onDelete: 'cascade' }),
  siteId: integer('site_id').notNull().references(() => sites.id, { onDelete: 'cascade' }),
  ...
  poolSiteIdx: index('site_pool_members_pool_site_idx').on(table.poolId, table.siteId),
  poolRoleIdx: index('site_pool_members_pool_role_idx').on(table.poolId, table.role, table.sortOrder),
  siteIdx: index('site_pool_members_site_id_idx').on(table.siteId),
  accountIdx: index('site_pool_members_account_id_idx').on(table.accountId),
  tokenIdx: index('site_pool_members_token_id_idx').on(table.tokenId),
}));
...
export const downstreamApiKeys = sqliteTable('downstream_api_keys', {
  ...
  assignmentMode: text('assignment_mode').notNull().default('custom'),
  sitePoolId: integer('site_pool_id').references(() => sitePools.id, { onDelete: 'set null' }),
  enabled: integer('enabled', { mode: 'boolean' }).default(true),
  ...
  assignmentModeIdx: index('downstream_api_keys_assignment_mode_idx').on(table.assignmentMode),
  sitePoolIdx: index('downstream_api_keys_site_pool_id_idx').on(table.sitePoolId),
}));
