...
import {
  listRealtimeMonitorCandidates,
  probeRealtimeMonitorCandidate,
} from '../../services/modelRealtimeMonitorService.js';
...
const limitMonitorModels = createRateLimitGuard({
  bucket: 'monitor-models',
  max: 30,
  windowMs: 60_000,
});

const limitMonitorProbe = createRateLimitGuard({
  bucket: 'monitor-model-probe',
  max: 20,
  windowMs: 60_000,
});
...
export async function monitorRoutes(app: FastifyInstance) {
  app.get('/api/monitor/models', { preHandler: [limitMonitorModels] }, async (request) => {
    const limit = parseOptionalPositiveInteger((request.query as Record<string, unknown>).limit) || 80;
    const candidates = await listRealtimeMonitorCandidates(Math.min(200, limit));
  ...
  app.post<{ Body: unknown }>(
    '/api/monitor/models/probe',
    { preHandler: [limitMonitorProbe] },
    async (request, reply) => {
      const body = (request.body && typeof request.body === 'object')
        ? request.body as Record<string, unknown>
        : {};
      const result = await probeRealtimeMonitorCandidate({
        ...
        siteId: parseOptionalPositiveInteger(body.siteId),
        accountId: parseOptionalPositiveInteger(body.accountId),
        timeoutMs: parseOptionalPositiveInteger(body.timeoutMs),
      });
      if (!result) {
        return reply.code(404).send({
          success: false,
          message: '没有找到可用于实时监测的模型，请先同步模型或绑定可用 API Key。',
        });
      }
