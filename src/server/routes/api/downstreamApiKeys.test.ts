...
describe('downstream api keys routes', () => {
  ...
  it('creates site pools and binds downstream keys to pool assignment', async () => {
    const site = await db.insert(schema.sites).values({
      name: 'pool-site',
    ...
    const poolRes = await app.inject({
      ...
      payload: {
        ...
        members: [{ siteId: site.id, role: 'primary', weight: 2 }],
      },
    });

    expect(poolRes.statusCode).toBe(200);
    const poolBody = poolRes.json();
    expect(poolBody).toMatchObject({
      success: true,
      item: {
        name: 'Project Pool',
        strategy: 'stable_first',
    ...
    expect(poolBody.item.members).toHaveLength(1);
    ...
    const createKeyRes = await app.inject({
      ...
      payload: {
        name: 'pool-bound-key',
        key: 'sk-pool-bound-key',
        assignmentMode: 'pool',
        sitePoolId: poolBody.item.id,
      },
    });

    expect(createKeyRes.statusCode).toBe(200);
    expect(createKeyRes.json()).toMatchObject({
      success: true,
      item: {
        name: 'pool-bound-key',
        assignmentMode: 'pool',
  ...
  it('rejects pool assignment without a valid site pool', async () => {
    ...
    expect(response.json().message).toContain('sitePoolId 不存在');
  });
