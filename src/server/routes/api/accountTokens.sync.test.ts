...
describe('account tokens sync routes with site status', () => {
  ...
  it('returns model availability summary for account tokens', async () => {
    const { account } = await seedAccount({ siteStatus: 'active' });
    const token = await db.insert(schema.accountTokens).values({
      accountId: account.id,
      name: 'route-token',
      token: 'sk-route-token',
    ...
    await db.insert(schema.tokenModelAvailability).values([
      {
        ...
        available: true,
        latencyMs: 320,
        checkedAt: '2026-05-14T01:00:00.000Z',
      },
      {
        tokenId: token.id,
        modelName: 'gpt-5.4',
        available: false,
        latencyMs: 900,
        checkedAt: '2026-05-14T01:05:00.000Z',
      },
    ]).run();
    ...
    expect(response.json()[0]).toMatchObject({
      ...
      modelAvailability: {
        ...
        unavailable: 1,
        unchecked: 0,
        lastCheckedAt: '2026-05-14T01:05:00.000Z',
        bestLatencyMs: 320,
      },
      availableModelsPreview: ['gpt-4.1'],
    });
  });
