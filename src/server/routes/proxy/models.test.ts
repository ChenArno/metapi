...
describe('/v1/models route', () => {
  ...
  it('returns only selected route models for managed key with allowedRouteIds policy', async () => {
    const site = await db.insert(schema.sites).values({
      name: 'test-site',
      url: 'https://upstream.example.com',
      platform: 'openai',
    ...
    await db.insert(schema.downstreamApiKeys).values({
      ...
      enabled: true,
      allowedRouteIds: JSON.stringify([allowedRoute.id]),
    }).run();
  ...
  it('does not add model restrictions when managed key has no selected model groups', async () => {
    const site = await db.insert(schema.sites).values({
      name: 'deny-all-site',
  ...
  it('expands selected explicit group into source exact models for managed key model list', async () => {
    const site = await db.insert(schema.sites).values({
  ...
  it('filters search pseudo models out of /v1/models', async () => {
    ...
    await db.insert(schema.downstreamApiKeys).values({
      ...
      key: 'sk-search-key',
      enabled: true,
      allowedRouteIds: JSON.stringify([llmRoute.id]),
    }).run();
