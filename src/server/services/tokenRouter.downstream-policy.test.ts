...
describe('TokenRouter downstream policy', () => {
  ...
  it('filters channels outside the downstream site pool', async () => {
    const siteAllowed = await db.insert(schema.sites).values({
      name: 'pool-allowed-site',
      url: 'https://pool-allowed.example.com',
      platform: 'new-api',
      status: 'active',
    }).returning().get();
    const siteBlocked = await db.insert(schema.sites).values({
      name: 'pool-blocked-site',
      url: 'https://pool-blocked.example.com',
      platform: 'new-api',
      status: 'active',
    }).returning().get();
    const accountAllowed = await db.insert(schema.accounts).values({
      siteId: siteAllowed.id,
      username: 'pool-allowed',
      accessToken: 'access-pool-allowed',
      apiToken: 'sk-pool-allowed',
      status: 'active',
      balance: 100,
    }).returning().get();
    const accountBlocked = await db.insert(schema.accounts).values({
      siteId: siteBlocked.id,
      username: 'pool-blocked',
      accessToken: 'access-pool-blocked',
      apiToken: 'sk-pool-blocked',
      status: 'active',
      balance: 100,
    }).returning().get();
    ...
    const selected = await router.selectChannel('gpt-4o-mini', policy);
    ...
    expect(decision.candidates.find((candidate) => candidate.channelId === allowedChannel.id)?.eligible).toBe(true);
    ...
    expect(blockedCandidate?.reason).toContain('站点不在下游密钥站点池');
  });
  ...
  it('uses only allowedRouteIds as the downstream model group scope', async () => {
    const site = await db.insert(schema.sites).values({
      name: 'site-union',
    ...
    expect(gptPick).toBeNull();
  });

  it('allows direct internal model selection through explicit group allowedRouteIds', async () => {
    const site = await db.insert(schema.sites).values({
      name: 'site-explicit-group',
    ...
    const account = await db.insert(schema.accounts).values({
      siteId: site.id,
      username: 'user-explicit-group',
      accessToken: 'access-explicit-group',
      apiToken: 'sk-explicit-group',
      status: 'active',
    }).returning().get();

    const internalRoute = await db.insert(schema.tokenRoutes).values({
    ...
    await db.insert(schema.routeGroupSources).values({
      ...
      sourceRouteId: internalRoute.id,
    }).run();

    await db.insert(schema.routeChannels).values({
      routeId: internalRoute.id,
      accountId: account.id,
      tokenId: null,
      priority: 0,
      weight: 10,
    ...
    const pickByAlias = await router.selectChannel('发发发', policy);
    const pickByInternalModel = await router.selectChannel('gpt-5.4', policy);

    expect(pickByAlias?.channel.routeId).toBe(internalRoute.id);
    expect(pickByInternalModel?.channel.routeId).toBe(internalRoute.id);
  });
