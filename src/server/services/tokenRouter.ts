...
function isModelAllowedByDownstreamPolicy(requestedModel: string, policy: DownstreamRoutingPolicy): boolean {
  ...
  if (!hasAllowedRoutes) return policy.denyAllWhenEmpty === true ? false : true;
...
export class TokenRouter {
   ...
   */
  async getAvailableModels(downstreamPolicy?: DownstreamRoutingPolicy): Promise<string[]> {
    const routes = await loadEnabledRoutes();
    const allowedRouteIds = Array.isArray(downstreamPolicy?.allowedRouteIds)
      ? downstreamPolicy.allowedRouteIds.filter((id) => Number.isFinite(id) && id > 0)
      : [];
    if (allowedRouteIds.length > 0) {
      const routeById = new Map(routes.map((route) => [route.id, route]));
      const exposed: string[] = [];
      for (const routeId of allowedRouteIds) {
        const route = routeById.get(routeId);
        if (!route) continue;
        if (isExplicitGroupRoute(route)) {
          const sourceNames = route.sourceRouteIds
            .map((sourceRouteId) => routeById.get(sourceRouteId))
            .filter((sourceRoute): sourceRoute is RouteRow => !!sourceRoute && !isExplicitGroupRoute(sourceRoute) && isExactRouteModelPattern(sourceRoute.modelPattern))
            .map((sourceRoute) => getExposedModelNameForRoute(sourceRoute).trim())
            .filter((name) => name.length > 0);
          if (sourceNames.length > 0) {
            exposed.push(...sourceNames);
            continue;
          }
        }
        const exposedName = getExposedModelNameForRoute(route).trim();
        if (exposedName) exposed.push(exposedName);
      }
  ...
  private async findRoute(model: string, downstreamPolicy: DownstreamRoutingPolicy): Promise<RouteMatch | null> {
    ...
    if (downstreamPolicy.allowedRouteIds.length > 0) {
      const allowSet = new Set(downstreamPolicy.allowedRouteIds);
      const expandedAllowSet = new Set<number>(downstreamPolicy.allowedRouteIds);
      for (const route of routes) {
        if (!allowSet.has(route.id) || !isExplicitGroupRoute(route)) continue;
        for (const sourceRouteId of route.sourceRouteIds) {
          if (!Number.isFinite(sourceRouteId) || sourceRouteId <= 0) continue;
          expandedAllowSet.add(sourceRouteId);
        }
      }
      routes = routes.filter((route) => expandedAllowSet.has(route.id));
    }
  ...
  private resolveDownstreamExclusionReason(
    ...
    const allowedSiteIds = Array.isArray(downstreamPolicy.allowedSiteIds)
      ...
      : [];
    if (downstreamPolicy.assignmentMode === 'pool' && allowedSiteIds.length === 0) {
      return '下游密钥绑定的站点池为空';
    }
