...
describe('modelAvailabilityProbeService', () => {
  ...
  it('falls back to manual or default models and seeds token availability rows when discovery is empty', async () => {
    const site = await db.insert(schema.sites).values({
      name: 'fallback-site',
