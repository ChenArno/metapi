...
describe('backupService', () => {
  ...
  it('deduplicates runtime model availability snapshots while importing account backups', async () => {
    const now = '2026-05-13T02:01:06.198Z';
    const site = await db.insert(schema.sites).values({
      name: 'dedupe-site',
    ...
    const account = await db.insert(schema.accounts).values({
      ...
      accessToken: 'dedupe-session-token',
      apiToken: null,
      balance: 0,
      balanceUsed: 0,
      quota: 0,
    ...
    const exported = {
      ...
      accounts: {
        ...
        manualModels: [
          {
            accountId: account.id,
            modelName: '[200k]deepseek-v4-flash',
          },
        ],
      },
    } as Record<string, unknown>;
    ...
    await db.insert(schema.modelAvailability).values([
      ...
      },
      {
        ...
        isManual: false,
        latencyMs: 512,
        checkedAt: '2026-05-13T03:01:06.198Z',
      },
    ]).onConflictDoUpdate({
    ...
    const restoredAvailability = await db.select()
      .from(schema.modelAvailability)
      .where(eq(schema.modelAvailability.accountId, account.id))
      .orderBy(asc(schema.modelAvailability.modelName))
      .all();

    expect(restoredAvailability).toEqual([
      expect.objectContaining({
        accountId: account.id,
        modelName: '[200k]deepseek-v4-flash',
        isManual: true,
