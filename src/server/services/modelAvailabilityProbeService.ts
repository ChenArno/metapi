...
const DEFAULT_FALLBACK_PROBE_MODELS = ['gpt-5.4'];
    ...
    if (target.rowId) {
      ...
      await db.insert(schema.modelAvailability)
        ...
        .onConflictDoUpdate({
          ...
          set: {
            ...patch,
            ...(target.isManual === true ? { isManual: true } : {}),
          },
        })
        .run();
    }
...
async function loadProbeTargetsForAccount(context: ProbeAccountContext): Promise<ProbeTarget[]> {
  ...
  const fallbackModelNames = Array.from(new Set([
    ...accountModels
      .map((row) => String(row.modelName || '').trim())
      .filter(Boolean),
    ...DEFAULT_FALLBACK_PROBE_MODELS,
  ]));

  const manualAccountModels = accountModels.filter((row) => row.isManual);
  for (const row of manualAccountModels) {
    targets.push({
      kind: 'account',
      rowId: row.id,
      modelName: row.modelName,
  ...
  const readyTokens = await db.select()
