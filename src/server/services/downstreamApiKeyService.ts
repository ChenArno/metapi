...
export function normalizeDownstreamAssignmentModeInput(input: unknown): DownstreamAssignmentMode {
  const value = typeof input === 'string' ? input.trim().toLowerCase() : '';
  if (value === 'auto' || value === 'pool' || value === 'custom') return value;
  return 'custom';
}
...
async function isModelMatchedByAllowedRoutes(model: string, allowedRouteIds: number[]): Promise<boolean> {
  ...
  if (routes.some((route) => getExposedRouteName(route) === model || matchesDownstreamModelPattern(model, route.modelPattern))) {
    return true;
  }

  const explicitGroupRouteIds = routes
    .filter((route) => String(route.routeMode || '').trim() === 'explicit_group')
    .map((route) => route.id);
  if (explicitGroupRouteIds.length === 0) return false;
  ...
  const sourceRows = await db.select({
    ...
    .all();
  const sourceRouteIds: number[] = Array.from(new Set(sourceRows
    .map((row) => Number(row.sourceRouteId))
    .filter((routeId): routeId is number => Number.isFinite(routeId) && routeId > 0)));
  if (sourceRouteIds.length === 0) return false;
  ...
  const sourceRoutes = await db.select({
    ...
    .from(schema.tokenRoutes)
    .where(and(
      inArray(schema.tokenRoutes.id, sourceRouteIds),
      eq(schema.tokenRoutes.enabled, true),
    ))
    .all();

  return sourceRoutes.some((route) => getExposedRouteName(route) === model || matchesDownstreamModelPattern(model, route.modelPattern));
}
...
export async function isModelAllowedByPolicyOrAllowedRoutes(model: string, policy: DownstreamRoutingPolicy): Promise<boolean> {
  ...
  if (!hasRouteRules) return policy.denyAllWhenEmpty === true ? false : true;
...
export function toDownstreamApiKeyPolicyView(row: DownstreamApiKeyRow): DownstreamApiKeyPolicyView {
  ...
  return {
    ...
    assignmentMode: normalizeDownstreamAssignmentModeInput(row.assignmentMode),
    sitePoolId: normalizePositiveIntegerOrNull(row.sitePoolId),
    enabled: !!row.enabled,
...
async function resolveAllowedSiteIdsForPool(sitePoolId: number | null): Promise<number[]> {
  if (!sitePoolId) return [];
  const rows = await db.select({
    siteId: schema.sitePoolMembers.siteId,
  })
    .from(schema.sitePoolMembers)
    .innerJoin(schema.sitePools, eq(schema.sitePoolMembers.poolId, schema.sitePools.id))
    .innerJoin(schema.sites, eq(schema.sitePoolMembers.siteId, schema.sites.id))
    .where(and(
      eq(schema.sitePoolMembers.poolId, sitePoolId),
      eq(schema.sitePools.enabled, true),
...
export async function buildPolicyForDownstreamKey(
  view: Pick<DownstreamApiKeyPolicyView,
    'assignmentMode'
    | 'sitePoolId'
    | 'supportedModels'
    | 'allowedRouteIds'
    ...
    | 'excludedCredentialRefs'
  >,
): Promise<DownstreamRoutingPolicy> {
  const assignmentMode = normalizeDownstreamAssignmentModeInput(view.assignmentMode);
  const sitePoolId = assignmentMode === 'pool' ? normalizePositiveIntegerOrNull(view.sitePoolId) : null;
  const allowedSiteIds = assignmentMode === 'pool'
    ? await resolveAllowedSiteIdsForPool(sitePoolId)
    : [];
...
export async function authorizeDownstreamToken(token: string): Promise<DownstreamTokenAuthResult> {
  ...
  if (managed) {
    ...
    return {
      ...
      key: managed,
      policy: await buildPolicyForDownstreamKey(managed),
    };
  }
...
export function normalizeDownstreamApiKeyPayload(input: {
  ...
  const assignmentMode = normalizeDownstreamAssignmentModeInput(input.assignmentMode);
  const sitePoolId = assignmentMode === 'pool'
    ? normalizePositiveIntegerOrNull(input.sitePoolId)
    : null;
