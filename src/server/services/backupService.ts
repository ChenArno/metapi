...
function buildAccountModelAvailabilityKey(accountId: number, modelName: string): string {
  return `${accountId}::${asString(modelName)}`;
}

function buildTokenModelAvailabilityKey(tokenId: number, modelName: string): string {
  return `${tokenId}::${asString(modelName)}`;
}
...
async function importAccountsSection(section: AccountsBackupSection): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    ...
    const insertedModelAvailabilityKeys = new Set<string>();
    if (shouldReplaceManualModels) {
      ...
      for (const row of section.manualModels || []) {
        ...
        const availabilityKey = buildAccountModelAvailabilityKey(row.accountId, row.modelName);
        if (insertedModelAvailabilityKeys.has(availabilityKey)) continue;
        insertedModelAvailabilityKeys.add(availabilityKey);
    ...
    for (const row of runtimeState.nonManualAvailability) {
      ...
      const availabilityKey = buildAccountModelAvailabilityKey(accountId, row.modelName);
      if (insertedModelAvailabilityKeys.has(availabilityKey)) continue;
      insertedModelAvailabilityKeys.add(availabilityKey);
    ...
    const insertedTokenAvailabilityKeys = new Set<string>();
    for (const row of runtimeState.tokenAvailability) {
      ...
      if (!tokenId) continue;
      const availabilityKey = buildTokenModelAvailabilityKey(tokenId, row.modelName);
      if (insertedTokenAvailabilityKeys.has(availabilityKey)) continue;
      insertedTokenAvailabilityKeys.add(availabilityKey);
