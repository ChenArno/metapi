...
function prioritizeProbeEndpointCandidates(
  candidates: UpstreamEndpoint[],
  modelName: string,
): UpstreamEndpoint[] {
  if (!Array.isArray(candidates) || candidates.length <= 1) return candidates;
  if (!candidates.includes('chat')) return candidates;
  if (/^claude([-/]|$)/i.test(modelName.trim())) return candidates;
  return [
    'chat',
    ...candidates.filter((endpoint) => endpoint !== 'chat'),
  ];
}
...
export function isLikelyConversationModel(modelName: string): boolean {
...
export async function probeRuntimeModel(input: {
  ...
  try {
    const endpointCandidates = prioritizeProbeEndpointCandidates(
      await withTimeout(
      () => resolveUpstreamEndpointCandidates(
        {
          site: input.site,
