...
describe('probeRuntimeModel', () => {
  ...
  it('prefers chat completions first for generic openai-style conversation probes', async () => {
    ...
    buildUpstreamEndpointRequestMock.mockImplementation((input: { endpoint?: string; modelName?: string }) => ({
      path: input.endpoint === 'responses' ? '/v1/responses' : '/v1/chat/completions',
      headers: { 'content-type': 'application/json' },
      body: { model: input.modelName || 'gpt-5.4' },
      runtime: {
        executor: 'default',
    ...
    expect(buildUpstreamEndpointRequestMock).toHaveBeenNthCalledWith(1, expect.objectContaining({
      endpoint: 'chat',
      modelName: 'gpt-5.4',
    }));
  });

  it('keeps the upstream target url when a transport proxy is configured', async () => {
    resolveUpstreamEndpointCandidatesMock.mockResolvedValue(['chat']);
    resolveChannelProxyUrlMock.mockReturnValue('http://127.0.0.1:7890');
    buildUpstreamEndpointRequestMock.mockReturnValue({
      path: '/v1/chat/completions',
      headers: { 'content-type': 'application/json' },
      body: { model: 'gpt-5.4' },
    ...
    expect(dispatchRuntimeRequestMock).toHaveBeenCalledWith(expect.objectContaining({
      targetUrl: 'https://probe.example.com/v1/chat/completions',
    }));
  });
});
