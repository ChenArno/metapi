...
export async function listTokensWithRelations(accountId?: number) {
  ...
  const availabilityRows = tokenIds.length > 0
    ? await db.select()
      .from(schema.tokenModelAvailability)
      .where(inArray(schema.tokenModelAvailability.tokenId, tokenIds))
      .all()
    : [];
  const availabilityByTokenId = new Map<number, typeof availabilityRows>();
  for (const row of availabilityRows) {
    const bucket = availabilityByTokenId.get(row.tokenId) || [];
    bucket.push(row);
    availabilityByTokenId.set(row.tokenId, bucket);
  }
  ...
  return rows
    ...
    const modelRows = availabilityByTokenId.get(row.account_tokens.id) || [];
    const availableModels = modelRows
      .filter((modelRow) => modelRow.available === true)
      .map((modelRow) => modelRow.modelName)
      .sort((left, right) => left.localeCompare(right));
    const checkedAtValues = modelRows
      .map((modelRow) => modelRow.checkedAt)
      .filter((value): value is string => typeof value === 'string' && value.length > 0)
      .sort();
    const latencyValues = modelRows
      .map((modelRow) => modelRow.latencyMs)
    ...
    return {
      ...
      modelAvailability: {
        ...
        unavailable: modelRows.filter((modelRow) => modelRow.available === false).length,
        unchecked: modelRows.filter((modelRow) => modelRow.available !== true && modelRow.available !== false).length,
        lastCheckedAt: checkedAtValues.length > 0 ? checkedAtValues[checkedAtValues.length - 1] : null,
        bestLatencyMs: latencyValues.length > 0 ? Math.min(...latencyValues) : null,
      },
      availableModelsPreview: availableModels.slice(0, 8),
      account: {
        id: row.accounts.id,
