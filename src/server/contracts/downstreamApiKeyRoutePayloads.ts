...
const downstreamApiKeyPayloadSchema = z.object({
  ...
  assignmentMode: z.enum(['auto', 'pool', 'custom']).optional(),
  sitePoolId: z.union([z.number(), z.string(), z.null()]).optional(),
...
function formatDownstreamApiKeyPayloadError(error: z.ZodError): string {
  ...
  if (firstPath === 'assignmentMode') {
    return 'Invalid assignmentMode. Expected auto, pool, or custom.';
  }
  if (firstPath === 'sitePoolId') {
    return 'Invalid sitePoolId. Expected number, string, or null.';
  }
