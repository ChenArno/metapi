...
export const api = {
  ...
  probeModelAvailability: (data?: { accountId?: number; wait?: boolean }) =>
  ...
  getSitePools: () => request("/api/site-pools"),
  createSitePool: (data: any) => request("/api/site-pools", {
    method: "POST",
    body: JSON.stringify(data),
  }),
  updateSitePool: (id: number, data: any) => request(`/api/site-pools/${id}`, {
    method: "PUT",
    body: JSON.stringify(data),
  }),
  deleteSitePool: (id: number) => request(`/api/site-pools/${id}`, {
    method: "DELETE",
  }),
  ...
  // Model realtime monitor
  getMonitorModels: (options?: { limit?: number }) => {
    const params = new URLSearchParams();
    if (options?.limit) params.set("limit", String(options.limit));
    const query = params.toString();
    return request(`/api/monitor/models${query ? `?${query}` : ""}`);
  },
  probeMonitorModel: (data?: {
    candidateId?: string;
    modelName?: string;
    siteId?: number;
    accountId?: number;
    ...
    request("/api/monitor/models/probe", {
      method: "POST",
      body: JSON.stringify(data || {}),
      timeoutMs: data?.timeoutMs ? data.timeoutMs + 5_000 : 35_000,
    }),

  // Legacy monitor embed config
