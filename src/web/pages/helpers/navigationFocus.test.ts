...
describe('navigationFocus helpers', () => {
  it('builds focus paths for site and account', () => {
    ...
    expect(buildAccountFocusPath(34, { segment: 'apikey' })).toBe('/accounts?segment=apikey&focusAccountId=34');
    expect(buildAccountFocusPath(-1)).toBe('/accounts');
    expect(buildTokenFocusPath(56)).toBe('/tokens?focusTokenId=56');
    expect(buildTokenFocusPath(0)).toBe('/tokens');
  });
