...
import { isModelGroupRoute } from '../helpers/tokenRouteClassification.js';
...
export type DownstreamSitePoolOption = {
  id: number;
  name: string;
...
function summarizeCallableAliases(routeIds: number[], routeOptions: RouteSelectorItem[]): string {
  const aliasSet = new Set<string>();
  for (const routeId of routeIds) {
    const route = routeOptions.find((item) => item.id === routeId);
    if (!route) continue;
    const alias = routeTitle(route).trim();
    if (!alias) continue;
    aliasSet.add(alias);
  }
  const aliases = Array.from(aliasSet);
  if (aliases.length === 0) return '未限制';
  if (aliases.length <= 4) return aliases.join('、');
  return `${aliases.slice(0, 4).join('、')} 等 ${aliases.length} 个`;
}
...
export default function DownstreamKeyEditorModal({
  ...
  sitePoolOptions: DownstreamSitePoolOption[];
}) {
  ...
  const groupRouteOptions = useMemo(
    () => routeOptions.filter(isModelGroupRoute),
    [routeOptions],
  );
  ...
  const selectedSitePool = sitePoolOptions.find((pool) => String(pool.id) === form.sitePoolId);
  const authorizationSummary = selectedGroupCount === 0
    ? '不限模型'
    : `允许 ${selectedGroupCount} 个模型群组`;
  const callableAliasSummary = summarizeCallableAliases(normalizedSelectedGroupRouteIds, groupRouteOptions);
  ...
  return (
    ...
    >
      <div className="info-tip" style={{ marginBottom: 0 }}>
        常规场景只需要填写名称、密钥和站点分配；模型范围统一用模型群组管理。
      </div>
      ...
      <div className="downstream-key-modal-field downstream-key-modal-field-full">
        <div className="downstream-key-modal-label">站点分配</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 8 }}>
          {[
            { value: 'auto', title: '自动分配', desc: '从所有健康可用站点中选择' },
            { value: 'pool', title: '绑定站点池', desc: '只在指定站点池内分配' },
            ...(form.assignmentMode === 'custom' ? [{ value: 'custom', title: '兼容旧自定义', desc: '建议改为绑定站点池' }] : []),
          ].map((item) => (
            <button
              key={item.value}
              type="button"
              className={`btn ${form.assignmentMode === item.value ? 'btn-primary' : 'btn-ghost'}`}
              style={{ justifyContent: 'flex-start', alignItems: 'flex-start', flexDirection: 'column', gap: 4, minHeight: 72, border: '1px solid var(--color-border)' }}
              onClick={() => onChange((prev) => ({
                ...prev,
                assignmentMode: item.value as DownstreamKeyEditorForm['assignmentMode'],
                sitePoolId: item.value === 'pool' ? prev.sitePoolId : '',
              }))}
            >
              <span style={{ fontWeight: 700 }}>{item.title}</span>
              <span style={{ fontSize: 12, opacity: 0.78 }}>{item.desc}</span>
            </button>
          ))}
        </div>
        {form.assignmentMode === 'pool' ? (
          <div style={{ marginTop: 10 }}>
            <select
              value={form.sitePoolId}
              onChange={(e) => onChange((prev) => ({ ...prev, sitePoolId: e.target.value }))}
              style={inputStyle}
            >
              <option value="">选择站点池</option>
              {sitePoolOptions.map((pool) => (
                <option key={pool.id} value={pool.id}>
                  {pool.name}{pool.memberCount !== undefined ? ` · ${pool.memberCount} 个成员` : ''}
                </option>
              ))}
            </select>
            <div className="downstream-key-modal-help">
              {selectedSitePool ? `当前绑定：${selectedSitePool.name}` : '未选择站点池时保存会失败。站点池用于把某个客户端限制在一组上游站点内。'}
            </div>
          </div>
        ) : (
          <div className="downstream-key-modal-help">
            {form.assignmentMode === 'auto' ? '默认推荐：自动使用所有健康且支持目标模型的站点。' : '旧自定义规则仍会保存，建议迁移为站点池。'}
          </div>
        )}
      </div>

      <div className="downstream-key-modal-field downstream-key-modal-field-full">
        <div className="downstream-key-auth-summary">
          <div>
            <div className="downstream-key-modal-label" style={{ marginBottom: 4 }}>绑定模型群组</div>
            <div className="downstream-key-modal-help">{authorizationSummary}。客户端请求时，`model` 要填这里选中的群组名。</div>
            <div className="downstream-key-modal-help" style={{ marginTop: 4 }}>
              当前调用模型预览：<span style={{ color: 'var(--color-text-primary)' }}>{callableAliasSummary}</span>
            </div>
          </div>
          <a className="btn btn-ghost" style={{ border: '1px solid var(--color-border)', whiteSpace: 'nowrap' }} href="/model-groups">
            管理模型群组
          </a>
        </div>
        <div className="toolbar-search" style={{ maxWidth: '100%', marginTop: 10 }}>
        ...
        <div style={{ maxHeight: 280, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
          {filteredGroups.length === 0 ? (
            <div style={{ fontSize: 12, color: 'var(--color-text-muted)', lineHeight: 1.6 }}>
              {groupRouteOptions.length === 0
                ? '暂无可绑定模型群组。请先点击“管理模型群组”创建通配符或正则群组。'
                : '暂无匹配群组，请换个关键词搜索。'}
            </div>
              ...
              <label key={route.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer', padding: '8px 10px', borderRadius: 10, border: '1px solid var(--color-border-light)', background: checked ? 'color-mix(in srgb, var(--color-primary) 10%, var(--color-bg-card))' : 'var(--color-bg-card)' }}>
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ marginTop: 4, fontSize: 11, color: 'var(--color-text-muted)' }}>
                    调用名：<code>{routeTitle(route)}</code>
                  </div>
