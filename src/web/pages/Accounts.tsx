...
type ManagedConnectionsSegment = Exclude<ConnectionsSegment, "tokens">;

const ACCOUNT_SEGMENTS: Array<{
  value: ManagedConnectionsSegment;
  label: string;
  tooltip: string;
...
function resolveDeprecatedAccountsRedirect(search: string): string | null {
  ...
  return `/tokens${nextSearch ? `?${nextSearch}` : ""}`;
}

type AccountsConnectionSurfaceProps = {
  forcedSegment?: ManagedConnectionsSegment;
  pageTitle?: string;
  showSegmentSwitcher?: boolean;
};

function AccountsConnectionSurface({
  forcedSegment,
  pageTitle = "链接管理",
  showSegmentSwitcher = true,
}: AccountsConnectionSurfaceProps = {}) {
  const location = useLocation();
  const navigate = useNavigate();
  const deprecatedRedirect = forcedSegment
    ? null
    : resolveDeprecatedAccountsRedirect(location.search);
  const activeSegment = useMemo(
    () => forcedSegment || resolveConnectionsSegment(location.search),
    [forcedSegment, location.search],
  );
  ...
  const setSegment = (nextSegment: ManagedConnectionsSegment) => {
  ...
  if (deprecatedRedirect) {
    return <Navigate to={deprecatedRedirect} replace />;
  }

  return (
    <div className="animate-fade-in">
      <div className="page-header">
        <h2 className="page-title">{tr(pageTitle)}</h2>
...
export default function Accounts() {
  return <AccountsConnectionSurface />;
}
