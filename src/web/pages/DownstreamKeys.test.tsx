...
beforeEach(() => {
  ...
  apiMock.getRoutesLite.mockResolvedValue([
    ...
    { id: 15, modelPattern: 'opencai', displayName: 'opencai', routeMode: 'explicit_group', enabled: true },
  ]);
  apiMock.getSitePools.mockResolvedValue({ success: true, items: [] });
...
describe('DownstreamKeys page', () => {
  ...
  it('shows callable model aliases from selected model groups', async () => {
    apiMock.getDownstreamApiKeysSummary.mockResolvedValue({
      success: true,
      items: [
        buildSummaryItem({ allowedRouteIds: [15] }),
      ],
    });
    apiMock.getDownstreamApiKeys.mockResolvedValue({
      success: true,
      items: [
        buildRawItem({ allowedRouteIds: [15] }),
      ],
    });
  ...
  it('supports create flow and drawer trend loading', async () => {
    ...
    try {
      ...
      expect(collectText(root!.root)).toContain('绑定模型群组');
      expect(collectText(root!.root)).not.toContain('高级配置');
      expect(collectText(root!.root)).not.toContain('站点倍率 JSON');
      expect(collectText(root!.root)).not.toContain('请求额度');
      expect(collectText(root!.root)).not.toContain('成本额度');
      expect(collectText(root!.root)).not.toContain('过期时间');
      expect(collectText(root!.root)).not.toContain('备注说明');
      expect(collectText(root!.root)).not.toContain('标签（可选）');

      const inputs = root!.root.findAllByType('input');
      expect(inputs.some((node) => node.props.placeholder === '例如：VIP / 内部项目 / A组')).toBe(false);
  ...
  it('keeps downstream authorization focused on model groups', async () => {
    let root!: WebTestRenderer;
    try {
      await act(async () => {
        root = create(
      ...
      expect(text).toContain('绑定模型群组');
      expect(text).not.toContain('模型白名单');
      expect(text).not.toContain('排除站点');
      expect(text).toContain('默认群组');
      expect(text).toContain('opencai');
      expect(text).not.toContain('GPT 4.1 Mini');
    } finally {
      root?.unmount();
    }
  });

  it('lets operators explicitly select all model groups before saving', async () => {
    apiMock.getRoutesLite.mockResolvedValue([
      { id: 11, modelPattern: 'claude-*', displayName: '默认群组', enabled: true },
      { id: 12, modelPattern: 'gpt-4.1-mini', displayName: 'GPT 4.1 Mini', enabled: true },
    ...
    try {
      ...
      const groupSelectAllBtn = root!.root.findAll((node) => node.type === 'button' && collectText(node).includes('全选'))[0];
  ...
  it('defaults new keys to unrestricted model and site scope before saving', async () => {
    apiMock.getRoutesLite.mockResolvedValue([
      { id: 11, modelPattern: 'claude-*', displayName: '默认群组', enabled: true },
      { id: 12, modelPattern: 'gpt-4.1-mini', displayName: 'GPT 4.1 Mini', enabled: true },
      { id: 13, modelPattern: 're:^gemini-2\\..*$', displayName: 'Gemini 全家桶', enabled: true },
    ...
    try {
      ...
      expect(collectText(root!.root)).toContain('不限模型');
      expect(collectText(root!.root)).toContain('已选 0 个群组');
  ...
  it('does not expose legacy exclusion controls in the downstream key editor', async () => {
    let root!: WebTestRenderer;
    try {
      await act(async () => {
        root = create(
      ...
      expect(text).not.toContain('默认 API Key');
