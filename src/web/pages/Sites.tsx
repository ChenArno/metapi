...
export default function Sites() {
  ...
  const proxyMode = form.useSystemProxy
    ? 'system'
    : form.proxyUrl.trim()
      ? 'custom'
      : 'direct';
  const systemProxyConfigured = systemProxyUrl.trim().length > 0;
  const maskedSystemProxyUrl = systemProxyConfigured
    ? systemProxyUrl.trim().replace(/:\/\/([^:@/]+):([^@/]+)@/, '://$1:****@')
    : '';
  ...
  useEffect(() => {
    ...
    api.getRuntimeSettings()
      .then((runtime: any) => setSystemProxyUrl(typeof runtime?.systemProxyUrl === 'string' ? runtime.systemProxyUrl : ''))
      .catch(() => setSystemProxyUrl(''));
  }, []);
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      proxyUrl: form.useSystemProxy ? '' : form.proxyUrl.trim(),
        ...
        >
          ...
          <ResponsiveFormGrid>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-text-primary)' }}>出站代理</div>
              <div style={{ display: 'grid', gap: 8 }}>
                {[
                  { value: 'direct', label: '直连', desc: '不使用代理，直接从当前服务器访问上游。' },
                  {
                    value: 'system',
                    label: '使用系统代理',
                    desc: systemProxyConfigured
                      ? `走设置页的系统代理：${maskedSystemProxyUrl}`
                      : '设置页尚未配置系统代理，保存后不会有代理可用。',
                  },
                  { value: 'custom', label: '单独代理', desc: '仅这个站点使用独立代理地址。' },
                ].map((option) => {
                  const active = proxyMode === option.value;
                  return (
                    <label
                      key={option.value}
                      style={{
                      ...
                      <input
                        ...
                        onChange={() => {
                          if (option.value === 'system') {
                            setForm((prev) => ({ ...prev, useSystemProxy: true, proxyUrl: '' }));
                          } else if (option.value === 'direct') {
                            setForm((prev) => ({ ...prev, useSystemProxy: false, proxyUrl: '' }));
                          } else {
                            setForm((prev) => ({ ...prev, useSystemProxy: false }));
                          }
                        }}
                      />
                      <span style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                        <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-text-primary)' }}>{option.label}</span>
                        <span style={{ fontSize: 12, color: 'var(--color-text-muted)', lineHeight: 1.5 }}>{option.desc}</span>
                      </span>
                    </label>
                  );
                })}
              ...
              {proxyMode === 'custom' && (
                <input
                  placeholder="单独代理地址，如 http://host:7890 或 socks5://host:1080"
                  value={form.proxyUrl}
                  onChange={(e) => setForm((prev) => ({ ...prev, proxyUrl: e.target.value, useSystemProxy: false }))}
                  style={formInputStyle}
                />
              )}
              <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>
                系统代理只需在设置页配置一次；站点这里选择是否走系统代理即可。
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <input
