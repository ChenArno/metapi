...
describe('ProxyLogs server-driven page', () => {
  ...
  it('refreshes both proxy log rows and summary immediately and during auto refresh polling', async () => {
    vi.useFakeTimers();
    let root!: WebTestRenderer;
    ...
    try {
      ...
      const initialQueryCalls = apiMock.getProxyLogs.mock.calls.length;
      const initialMetaCalls = apiMock.getProxyLogsMeta.mock.calls.length;

      const autoRefreshButton = root.root.find((node) => (
        node.type === 'button'
        && typeof node.props.onClick === 'function'
        && collectText(node).includes('自动刷新')
      ));

      await act(async () => {
        autoRefreshButton.props.onClick();
      });
      await flushMicrotasks();

      expect(apiMock.getProxyLogs.mock.calls.length).toBe(initialQueryCalls + 1);
      expect(apiMock.getProxyLogsMeta.mock.calls.length).toBe(initialMetaCalls + 1);
      ...
      expect(apiMock.getProxyLogs.mock.calls.length).toBe(initialQueryCalls + 2);
      expect(apiMock.getProxyLogsMeta.mock.calls.length).toBe(initialMetaCalls + 2);
      expect(apiMock.getProxyLogsMeta).toHaveBeenLastCalledWith({
        status: 'all',
        search: '',
        refresh: 1,
