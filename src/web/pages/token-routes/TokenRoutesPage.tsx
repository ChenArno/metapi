...
import { applyPriorityRailDrop, isPriorityRailNewLayerId } from './priorityRail.js';
import { useRouteChannels } from './useRouteChannels.js';
import RouteFilterBar, { type EnabledFilter } from './RouteFilterBar.js';
import ManualRoutePanel from './ManualRoutePanel.js';
...
export default function TokenRoutes({
  ...
  mode?: 'routes' | 'model-groups';
}) {
  const navigate = useNavigate();
  const groupOnlyMode = mode === 'model-groups';
  ...
  const listVisibleRoutes = useMemo(
    () => buildVisibleRouteList(visibleRouteRows, isExactModelPattern, matchesModelPattern)
      .filter((route) => (groupOnlyMode ? !isRouteExactModel(route) : true)),
    [visibleRouteRows, groupOnlyMode],
  );
    ...
    <div className="animate-fade-in" style={{ minHeight: 400 }}>
      {groupOnlyMode ? (
        <div className="page-header" style={{ marginBottom: 12 }}>
          <div>
            <h1 className="page-title">{tr('模型群组')}</h1>
            <div className="page-subtitle">
              {tr('集中管理对外模型别名、通配符和正则路由；下游密钥只负责选择是否允许使用这些群组。')}
            </div>
          </div>
        </div>
      ) : null}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
        <div className="toolbar-search" style={{ minWidth: 220, flex: 1, maxWidth: 360 }}>
          ...
          <input
            ...
            placeholder={groupOnlyMode ? tr('搜索模型群组...') : tr('搜索模型路由...')}
          />
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, borderLeft: '1px solid var(--color-border)', paddingLeft: 8 }}>
          ...
          <button
            ...
            style={{ border: '1px solid var(--color-border)', padding: '7px 12px' }}
          >
            {groupOnlyMode ? tr('新建模型群组') : tr('新建群组')}
          </button>
        ...
        <span className="badge badge-info" style={{ fontSize: 12, fontWeight: 500, marginLeft: 'auto' }}>
          {tr('共')} {filteredRoutes.length} {groupOnlyMode ? tr('个群组') : tr('条路由')}
        </span>
      </div>
      ...
      <ResponsiveFilterPanel
        ...
        mobileTitle={groupOnlyMode ? tr('筛选模型群组') : tr('筛选路由')}
