...
describe('Accounts create intent handling', () => {
  ...
  it('redirects create intent in the tokens segment to /tokens', async () => {
    let root!: WebTestRenderer;
    try {
      await act(async () => {
        root = create(
          <MemoryRouter initialEntries={['/accounts?segment=tokens&create=1&siteId=10']}>
            <ToastProvider>
              <Routes>
      ...
      const rendered = JSON.stringify(root.toJSON());
      expect(rendered).toContain('/tokens?create=1&siteId=10');
