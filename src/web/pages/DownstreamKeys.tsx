...
import { isExactModelPattern, isModelGroupRoute } from './helpers/tokenRouteClassification.js';
...
type DownstreamApiKeyItem = {
  ...
  tags: string[];
  assignmentMode?: 'auto' | 'pool' | 'custom';
...
function buildEditorForm(
  ...
  return {
    ...
    sitePoolId: item?.sitePoolId ? String(item.sitePoolId) : '',
...
function summarizeCallableModels(routeIds: number[], routeMap: Map<number, RouteSelectorItem>): string {
  if (!Array.isArray(routeIds) || routeIds.length === 0) return '未限制';
  ...
  if (aliases.length === 0) return `${routeIds.length} 个模型`;
  if (aliases.length === 1) return aliases[0];
  return `${aliases[0]} +${aliases.length - 1}`;
}
...
function summarizeAssignmentMode(item: Pick<DownstreamApiKeyItem, 'assignmentMode' | 'sitePoolId'>, sitePoolMap: Map<number, DownstreamSitePoolOption>): string {
  if (item.assignmentMode === 'pool') {
    const pool = item.sitePoolId ? sitePoolMap.get(item.sitePoolId) : null;
    return pool ? `站点池 · ${pool.name}` : '站点池 · 未选择';
  }
  if (item.assignmentMode === 'custom') return '高级自定义';
  return '自动分配';
}
...
export default function DownstreamKeys() {
  ...
  const [sitePoolOptions, setSitePoolOptions] = useState<DownstreamSitePoolOption[]>([]);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [summaryRes, rawRes, routesRes, sitePoolsRes] = await Promise.all([
      ...
      setSitePoolOptions((Array.isArray(sitePoolsRes?.items) ? sitePoolsRes.items : []).map((pool: any) => ({
        id: Number(pool.id),
        name: String(pool.name || `站点池 ${pool.id}`),
        strategy: pool.strategy,
        enabled: !!pool.enabled,
        memberCount: Array.isArray(pool.members) ? pool.members.length : 0,
      })).filter((pool: DownstreamSitePoolOption) => Number.isFinite(pool.id) && pool.id > 0));
    } catch (err: any) {
  ...
  const sitePoolMap = useMemo(() => new Map(sitePoolOptions.map((item) => [item.id, item])), [sitePoolOptions]);
  ...
  const managedItems = useMemo<ManagedItem[]>(() => (
    summaryItems.map((item) => {
      ...
      return {
        ...
        assignmentMode: raw?.assignmentMode ?? item.assignmentMode,
        sitePoolId: raw?.sitePoolId ?? item.sitePoolId,
  ...
  useEffect(() => {
    ...
    if (defaultSelections.selectedGroupRouteIds.length === 0) {
      return;
    }

    setEditorForm((prev) => {
      if (prev.selectedGroupRouteIds.length > 0) {
        return prev;
      }
      return {
        ...prev,
  ...
  const openCreate = () => {
    setEditingId(null);
    setEditorForm(buildEditorForm(null, routeOptions, false));
    setCreateDefaultsPending(false);
    setEditorOpen(true);
  };
  ...
  const saveKey = async () => {
    ...
    if (editorForm.assignmentMode === 'pool' && !editorForm.sitePoolId) {
      toast.info('请选择站点池');
      return;
    }
    ...
    try {
      const payload = {
        ...
        assignmentMode: editorForm.assignmentMode,
        sitePoolId: editorForm.assignmentMode === 'pool' ? Number(editorForm.sitePoolId) : null,
        ...
        supportedModels: [],
        allowedRouteIds: uniqIds(editorForm.selectedGroupRouteIds).filter((id) => routeMap.has(id) && isModelGroupRoute(routeMap.get(id)!)),
    ...
    <div className="animate-fade-in" style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div className="page-header" style={{ marginBottom: 0 }}>
        <div>
          ...
          <div className="page-subtitle">统一管理分发给下游项目的密钥、主分组、标签、额度、站点池、模型群组与历史用量。</div>
        </div>
            ...
            {visibleItems.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  <MobileField label="分配" value={summarizeAssignmentMode(row, sitePoolMap)} stacked />
                  <MobileField label="模型群组" value={summarizeRouteLimit(row.allowedRouteIds || [], routeMap)} stacked />
                  <MobileField label="调用模型" value={summarizeCallableModels(row.allowedRouteIds || [], routeMap)} stacked />
                  <MobileField label="分配倍率" value={summarizeSiteWeightMultipliers(row.siteWeightMultipliers || {})} stacked />
                    ...
                    <tr key={row.id} className={`row-selectable ${checked ? 'row-selected' : ''}`.trim()} onClick={() => { setSelectedId(row.id); setDrawerOpen(true); }}>
                      ...
                      <td>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                          <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>分配：<span style={{ color: 'var(--color-text-primary)' }}>{summarizeAssignmentMode(row, sitePoolMap)}</span></div>
                          <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>模型群组：<span style={{ color: 'var(--color-text-primary)' }}>{summarizeRouteLimit(row.allowedRouteIds || [], routeMap)}</span></div>
                          <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>调用模型：<span style={{ color: 'var(--color-text-primary)' }}>{summarizeCallableModels(row.allowedRouteIds || [], routeMap)}</span></div>
                          ...
                          <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>分配倍率：<span style={{ color: 'var(--color-text-primary)' }}>{summarizeSiteWeightMultipliers(row.siteWeightMultipliers || {})}</span></div>
                        </div>
                      </td>
      ...
      <DownstreamKeyEditorModal
        ...
        sitePoolOptions={sitePoolOptions}
      />
      ...
      >
        <div className="info-tip" style={{ marginBottom: 0 }}>
          本次会对已选中的 {selectedIds.length} 个密钥批量设置主分组，并追加标签。不会改动模型群组、站点池、额度和倍率。
        </div>
